...
BEGIN TRY 
	...
	EXEC sp_addClient @nameClient, @addressClient, @idClient = @idClient OUTPUT;
...
BEGIN
	...
	SET @idClient = (SELECT IdClient 
					 FROM Client WHERE Client.Name = @nameClient);
	DECLARE @idMagazine INT;
...
AS
BEGIN
	DECLARE @idClient INT, @amount INT;
	SET @idClient = (SELECT IdClient
					FROM Client WHERE Client.Name = @nameClient);
...
FROM  Magazine M 
INNER JOIN Subscriptions S ON M.IdMagazine = S.IdMagazine 
...
UNION
SELECT * FROM (SELECT DISTINCT TOP 1 P.FirstName AS Name, P.LastName, (SELECT COUNT(Subscriptions.IdUser) FROM Subscriptions
												WHERE Subscriptions.IdUser = U.IdUser) AS Amount
			FROM Person P 
			INNER JOIN Users U ON P.IdPerson = U.IdPerson
			INNER JOIN Subscriptions S ON U.IdUser = S.IdUser
			ORDER BY Amount DESC
			)AS Resutl2;
---------------------------------------------------------
...
DECLARE @idAddressOUT INT;
EXEC sp_addAddress 'CR', 'Cartago', 'Cartago', 12345, @idAddress = @idAddressOUT OUT;


DECLARE @idClientOUT INT;
EXEC sp_addClient 'Editorial UNED', 'San Jose', @idClient = @idClientOUT OUT; 


DECLARE @dateMagazineC DATE;
SET @dateMagazineC = '2017-11-16';
EXEC sp_addContract  'Editorial TEC', 'Cartago', 3, 350000, 2, 4, 'Libreria ASEIC', 'CR', 'Cartago', 'Cartago',
						23456, 'Revista Matematica', 2500, @dateMagazineC;


DECLARE @idLocalOUT INT;
EXEC sp_addLocal 'Librerias UNED', 'Costa Rica', 'San Jose', 'San Pedro', 45678, @idLocal = @idLocalOUT OUT;


DECLARE @idMagazineOUT INT, @dateMagazine DATE;
SET @dateMagazine = '2007-12-14';
EXEC sp_addMagazine 'Cultivo Hidroponico', 3500, @dateMagazine, @idMagazine = @idMagazineOUT OUT;


DECLARE @arriveDateLocal DATE;
SET @arriveDateLocal = '2017-06-25';
EXEC sp_addRegister 300001969, 'Local1', @arriveDateLocal;


EXEC sp_collectContract 'La Nacion';
EXEC sp_collectContract 'ASOC Noticas CR';


EXEC sp_collectInLocal 'Local5';


EXEC sp_collectOrderService 'Editorial TEC', 'Revista Matematica', 300001995, 20;
